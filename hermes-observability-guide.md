...
- ≈24h TTL **只绑 iLink prepare/发送路径**,实测边界 **(23.08, 24.17]h**(自最后入站起算)。「token 过期」应表述为「prepare 层会话过期」。
- **poll/入站路径不受 token issued_at 约束**:34.5h 龄 token 下 poll 心跳仍全绿(08-29 可证伪预测实锤:无入站 → 4 晨 cron 全败,同期 poll 零中断)。入站通道长活=「仅入站刷新/入站即恢复」的机制基础。
- **失败画像**:用户晚间不聊天 → token 不换发 → 次日起 cron 投递全败(job record 表象 `rate_limited` 误归;`send_result` 的 `classification=stale_session` 才是真值,交叉纠正会标),直到下次入站。晚间 22:00 cron 踩 ~23h 龄,**余量仅 ~1h**。
- `summary`/`token` 只报数值不做判定(TTL 是经验值);消费时自行对照:年龄逼近 23h 且即将有 cron 投递 → 提醒用户发消息换发(出站永不自愈,入站即恢复)。
