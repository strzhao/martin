...
- evidence review 的 offer 只在**域匹配且缺口驱动**时带（#86062 模式，措辞按 §11 署名排序规范）；**货不是动作的装饰**——库存没有域匹配的货，评审就纯 review，**不为带而带**（反 slop：#103650 教训，硬凑的 offer 连署名都会丢）；也不为凑 KPI 现场造货——真缺口才立项（forge），货是缺口的自然产物（09-11 用户拍板：旧「有货必带、没货不硬带」口径作废）
- 本地库存持续按「单关注点 + 可剥离 + mutation 自证 + 基于 current main」标准锻造，保持随时可 offer 状态
- 纯观察/纯评论的动作占比靠**选题质量**自然控制（rubric 五维筛选后本就少）；连续多个无 commit 产出的动作 = 形态报警，**复查的是选题与库存建设节奏，不是往单个 review 里塞货**
