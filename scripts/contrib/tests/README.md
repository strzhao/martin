...
- **删除类跨窗所有权链（D-α，`ownership-delete-ok`）**：D-β 近邻对「写手建新的、删旧的」这一生命周期形态**恒不成立**——删除时刻不落在任何具名记录的 ±30s 内（生产实测：`pending/digest-20260914-100442.*` 四件的最近在窗记录 `11:08:39` 与父目录锚 `11:09:18` 相距 39s ⇒ 该形态每小时假红一次）。D-α 在 D-β **之后**追加判定（D-β 成立就不求值）：**所有权** = 佐证日志里一条合规记录的**起始字节偏移 < 该日志在 before 快照中的 size**（= 窗口前字节区）∧ 该行文本含被删物 stem（`basename` 去掉首个 `.` 及其后；长度须 ≥ `WA_OWNER_MIN_STEM=8`，短于它 ⇒ 所有权不适用、fail-closed）；**活跃度** = 同一日志有 ts ∈ `[t0−120s, t1+120s]` 的合规记录（写手本窗在活动）。两者同时成立 ⇒ `external/ownership-delete-ok`；否则 ⇒ `suite/no-delete-corroboration`（失败行**追加** `owner=`/`act=` 审计字段）。
  - **为什么用「字节偏移」而非「记录时间戳 < t0」**：时间戳是记录的自述文本——在窗内补写一条自带旧时间戳的记录即可伪造；字节偏移是结构事实：窗口前区 = 该日志在 before 快照里的前 N 字节，套件在窗内追加的任何字节都落在 `offset >= b_size` 的后区，**伪造所有权在机制上不可达**。另有双重覆盖：被伪造的追加自身即被既有 append 分支判 `suite/timestamp-out-of-window`（append 要求块内至少一条 ts ∈ 窗口带）。
  - **D-β 逐字未动**：判定、reason、证据行字段与顺序不变；D-α 证据行新字段**一律追加在 `dir_mtime=` 之后**（既有 `_ev_ts` 的 `%% dir_mtime=` 截断与 `_ev_dirmtime` 贪婪语义因此不变）：`writer=… Δt=… ts=… dir_mtime=… owner=<stem> owner_ts=<ts> owner_log=<p> act_ts=<ts>`。三个内部助手：`wa__snap_field <snap> <relpath> <col>`（按第 5 列精确等值取列）/ `wa__owner_prior <repo> <corr 字段> <path> <before_snap>` / `wa__activity_in_window <repo> <corr 字段> <lo_key> <hi_key>`；`wa__classify_one` 增第 9 参 `<before_snap>`（内部函数，非公开契约）。
  - **判读口径**：失败行 `owner=<stem>` 有值 + `act=` 有值 ⇒ 所有权与活跃度都在，只是所有权记录不在窗口前字节区（查日志是否被重写/轮转）；`owner=none` ⇒ stem 短于 8 或该日志不在清单里；`act=none` ⇒ 写手本窗静默。
- **reason 令牌闭集**：external = `registered-append-ok` / `corroborated-ok` / `corroborated-delete-ok` / **`ownership-delete-ok`**；suite = `canary-marker` / `alphabet-violation` / `not-append-only` / `inode-changed` / `empty-append` / `timestamp-out-of-window` / `created-unallowed` / `deleted` / `no-corroboration` / **`no-delete-corroboration`**；outside = `outside-surface`。
