...
<!-- tags: hermes, kanban, profile, claude-code, autopilot, delegation, headless, decision -->
## [2026-09-08] hermes 复杂编码委派 coder profile 选型：真 profile 内转调 + 单条 claude -p 长进程
背景：hermes 自己做复杂编码质量差，Claude Code 侧 autopilot（state.md 状态机 + Stop hook 自循环）效果好。三候选对比后用户拍板：①**真 profile worker 内转调**（dispatcher 原生 spawn，worker 用 terminal(background=true) 跑 claude -p 再 kanban_complete）胜过 code-cc lane + 外部 claim 脚本（后者可靠性最高但要维护编排脚本）与源码定制 spawn（改动大）；②**单条 -p 长进程**（autopilot Stop hook 在 print 会话结束自续 phase）胜过多轮 -p 接力（实现简，靠 perl alarm 3h 硬顶兜无界）；③**AI 自判零硬闸**（用户拍板「AI 很聪明，自己判断」）。**worktree 归属 kanban 物化**：autopilot SessionStart hook 在主仓库（.git 是目录）静默退出、在 worktree（.git 是文件）自动 repair 进 worktree-session 模式（worktree-bootstrap.sh:22）——两层机制天然兼容，不需嵌套 worktree。关键防线：worker 便宜模型陪跑（k3）+ claude_attempts ≤ 2 + --disallowedTools 硬禁 push/gh 写 + max_in_progress_per_profile: 1（单个 int，所有 profile 每域串行——字典形态会被 dispatcher 拒绝 ignoring，kanban_watchers.py:1391 只做 int() 校验，按域字典限流是上游缺口=贡献候选）。**Phase 0 spike 实证（09-08 当日闭环）**：①无头全链可行——throwaway repo 3 轮 claude -p 接力跑完 goal→done（design/蓝红对抗/qa/merge/commit，--fast 全程零审批卡点）；②**「单条 -p 长进程」被证伪**：-p 进程退出后循环即停（iteration 不自增），必须调用方循环重调直到 state.md phase=done——等待 SOP 已改多轮接力；③启动轮 slash prompt 的 final output 常为空、续跑轮自然语言 prompt 输出完整可读——摘要不能依赖启动轮 stdout；④耗时基线：小任务全链 3 轮 ≈15-20min，design 轮最重，轮级 alarm 900s。

## [2026-09-11] zcode 插件手动四件套脚本化：sync-autopilot-to-zcode.sh（registry upsert 语义三约定）
背景：zcode CLI 0.16.5 无 marketplace add/install，本地插件装/升级靠 zcode 使用文档 §4.2 手动四件套。交付 martin `scripts/coder/sync-autopilot-to-zcode.sh` 自动化（--check exit 0/1 + 默认同步 + 幂等 + ZCODE_CLI_HOME 覆盖，JSON upsert 走 python3 stdlib）。写脚本前运行时实证的缓存语义（~/.zcode/cli/plugins/）：①双注册表 upsert 必须保留既有条目字段（description/addedAt/scope 是用户可见面），installedAt 仅版本变化或新建才刷新——否则二次运行字节漂移、幂等破坏；②旧版本实体目录 cache/<mkt>/<plugin>/<旧ver>/ 并存不删（对齐 CLI 自身行为）；③版本判定用字符串相等即可（任何不等=落后，无需 semver 序比较）。红队 52 断言 + 编排器独立 P1-P6 谓词求值全过。**[2026-09-13 已退役]**：coder lane 摘除 zcode 引擎（免费包烧空 + 慢 ~10× 贵 10-20×），本脚本与验收测试已从仓内删除（本条留档）；registry upsert 三约定仍适用于任何 JSON registry 幂等写入场景。
<!-- tags: zcode, plugin, marketplace, sync-script, registry-upsert, idempotency, bash, python3, deprecated -->

## [2026-09-11] 上游回馈闸门已投递判重选型：纯本地 git refs 头 patch-id，snapshot 扩展评估后不走
候选过滤条件 d「未已投递」三候选对比：①sha 相等比对——拒绝（cherry-pick/rebase 后 sha 必变，判不住真实重复）；②扩展 own-pr-watch snapshot 加 headRefOid 映射 PR 号——评估后不走（需改 gh 查询+三处 jq+T7 夹具联动，超增补半径；snapshot prs 元素现无 head 字段实证）；③**本地 git refs 头 patch-id**（采纳）：refs/heads/contrib/* ∪ refs/remotes/fork/* 头 commit patch-id 集合，fork remote-tracking ref 随 push 自动更新故零 gh 零网络；summary 记 ref 短名（fork/contrib/<branch>），PR 号映射留待 snapshot 自然演进。拦截方向刻意保守：任一领先 commit 命中即整卡拦截+事件留痕供人工复核——代价是偶发多拦，收益是杜绝上游双 PR 撞「Extend, don't duplicate」rubric。
<!-- tags: contrib, upstream, patch-id, dedup, decision, gate, own-pr, zero-network -->

## [2026-09-13] 值班环 L1 特权动作分工：worker 只判定声明、编排层代行执行（fence 下的白名单二分）
背景：hermes kanban worker 子进程继承 HERMES_DELEGATED_CHILD_CONTEXT=1，kanban 写动词（archive/create/complete）与 kanban_db 写事务 fail-closed（hermes_cli/kanban.py:214-241）——值班 worker 归档孤儿卡在机制上不可能，且禁 unset 闸、禁直连 SQLite、禁任何旁路（拆闸=拆框架）。选型：按「谁能执行」二分白名单——worker 可执行面=纯文件/自有工具动作（清 kanban-flight-*.json 陈旧登记〔仅登记卡终态或查无〕、rq.sh set <id> expired〔仅 premise 实证死亡且项态仍是深检自有态〕、rq.sh budget refund〔仅 budget 账本确有 reserve 记录，防双退=预算超发〕）；archive=worker 只在台账写 archive-request 声明行，由编排层 duty_card.sh apply（run-watch 值班段，非 worker 上下文）复核五条（板内存在 / blocked|gave_up / 非 running / 龄>24h / 所提 rq 无 awaiting-approval|approved）后代行归档并落 executed（或 skipped）台账行。分工同时写进模式七（worker 侧「为何归档不是我动手」）与 duty_card.sh 头注释（编排侧「为何 apply 是编排层专属」），两侧互为镜像防漂移。真库闭环实证（2026-09-13 06:20）：孤儿卡 t_cb14f0ee（blocked 76.6h、rq 已 executed）→ apply 归档 archived + 台账 executed 行 + 复跑幂等 skipped 行。
<!-- tags: hermes, kanban, fence, delegated-child, duty-loop, privilege-split, worker-orchestrator, fail-closed, contrib-watch -->
