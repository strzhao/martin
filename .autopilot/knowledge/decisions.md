...
背景：zcode CLI 0.16.5 无 marketplace add/install，本地插件装/升级靠 zcode 使用文档 §4.2 手动四件套。交付 martin `scripts/coder/sync-autopilot-to-zcode.sh` 自动化（--check exit 0/1 + 默认同步 + 幂等 + ZCODE_CLI_HOME 覆盖，JSON upsert 走 python3 stdlib）。写脚本前运行时实证的缓存语义（~/.zcode/cli/plugins/）：①双注册表 upsert 必须保留既有条目字段（description/addedAt/scope 是用户可见面），installedAt 仅版本变化或新建才刷新——否则二次运行字节漂移、幂等破坏；②旧版本实体目录 cache/<mkt>/<plugin>/<旧ver>/ 并存不删（对齐 CLI 自身行为）；③版本判定用字符串相等即可（任何不等=落后，无需 semver 序比较）。红队 52 断言 + 编排器独立 P1-P6 谓词求值全过。
<!-- tags: zcode, plugin, marketplace, sync-script, registry-upsert, idempotency, bash, python3 -->
