...
- **微信订阅摘除面已有对外车**（09-15 实查）：`gateway/kanban_watchers_notifier.py` 的「连续失败即摘除订阅」正被 **#111028**（Monti76，`fix(kanban): require a wall-clock window before dropping a notification sub`）改成加墙钟窗口 ⇒ §3.5「微信订阅会被网关永久摘除」那条缺口**已有他人持有车**，我方零动作（不帮扶、不 review、不并车）
