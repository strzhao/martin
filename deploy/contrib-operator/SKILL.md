...
- **`gh search prs|issues` 的静默空集有三种形态——都必须在「0 命中」时用 REST `total_count` + 独立控制组交叉验证后才能下结论（09-13/09-14 实证合并条）**：
  ...
  - **控制组必须至少两维**（这是唯一能当场识破 A/B/C 的手段）：①非日期维度 nonsense 短语 → 0（证明检索链路有效）②无日期限定的 broad 查询 → 大数（证明仓库/鉴权可达）。两者任一异常即停手排查，不得把 0 当结论。证据：shift-30 开班的 0 命中即由此识破。
